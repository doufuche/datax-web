...
public class JobTrigger {
     ...
     * @param total               sharding index
     */
    private static ReturnT<String> runTrigger(JobGroup group, JobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {
        ...
        if (address != null) {
            ...
            triggerResult = new ReturnT<String>(ReturnT.FAIL_CODE, "trigger_remote_executor_error");
        }
     ...
     *                              not null: cover job param
     */
    public static ReturnT<String> runTrigger(int jobId, TriggerTypeEnum triggerType, int failRetryCount, String executorShardingParam, String executorParam) {
        JobInfo jobInfo = JobAdminConfig.getAdminConfig().getJobInfoMapper().loadById(jobId);
        if (jobInfo == null) {
            logger.warn(">>>>>>>>>>>> trigger fail, jobId invalid，jobId={}", jobId);
            return null;
        ...
        if (ExecutorRouteStrategyEnum.SHARDING_BROADCAST == ExecutorRouteStrategyEnum.match(jobInfo.getExecutorRouteStrategy(), null)
                ...
                && shardingParam == null) {
            for (int i = 0; i < group.getRegistryList().size(); i++) {
                ReturnT<String> returnT = runTrigger(group, jobInfo, finalFailRetryCount, triggerType, i, group.getRegistryList().size());
                if(ReturnT.SUCCESS_CODE == returnT.getCode()){
                    logger.info("jobId:{}.remote.runTrigger.success!", jobId);
                    return returnT;
                }
            }
        } else {
            ...
            return runTrigger(group, jobInfo, finalFailRetryCount, triggerType, shardingParam[0], shardingParam[1]);
        }
        return null;
    }
}
